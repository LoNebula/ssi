...
{
	ssi_print("\n-------------------------------------------\n");
	ssi_print("TEST CONNECTION TO DATABASE\n\n");
	MongoURI uri(params.server, params.port, params.username, params.password);
	MongoClient client;
	if (client.connect(uri, params.database, false, 5000, params.tls, params.verify))
	{
		if (client.check())
		{
			ssi_print("\nSUCCESS: connected to database '%s'\n", params.database);
		}
		else
		{
			ssi_print("\nERROR: could not connect to database '%s' (server not reachable or invalid credentials)\n", params.database);
		}
		client.close();
	}
	else
	{
		ssi_print("\nERROR: could not initialize connection to database '%s'\n", params.database);
	}
}
...
int main(int argc, char **argv) {
#ifdef USE_SSI_LEAK_DETECTOR
	{
		...
		if (cmd.read(argc, argv)) {
			ssi_print("%s", info);
			ssi_print("[debug] server=%s port=%d database=%s\n", params.server, params.port, params.database);
			ssi_print("[debug] username=%s (null=%s, empty=%s)\n", params.username, params.username == NULL ? "yes" : "no", (params.username && params.username[0] == '\0') ? "yes" : "no");
			ssi_print("[debug] tls=%s verify=%s\n", params.tls ? "true" : "false", params.verify ? "true" : "false");
			...
			case 1: {
				ssi_print("[switch] entered case 1 (--test)\n");
...
{
	char c;
	char password[SSI_MAX_CHAR];
	char username[SSI_MAX_CHAR];
	int i = 0;

	if (params.username == NULL || params.username[0] == '\0' || ssi_strcmp(params.username, "(null)"))
	{
		printf("username: ");
		i = 0;
	...
	if (params.password == NULL || params.password[0] == '\0' || ssi_strcmp(params.password, "(null)"))
	{
		printf("\npassword: ");
		i = 0;
		while ((c = (char)_getch()) != '\r') {
